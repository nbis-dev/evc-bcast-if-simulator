...
function openfileif(filebtn) {
    if (!("files" in filebtn)) return;
    if (filebtn.files.length == 0) return;
...
function _resize() {
    ...
    screenlines =
        Math.floor(
            (window.innerHeight - t.offsetHeight) / d.firstChild.offsetHeight
        ) - 1;
}
...
function _render(increment, repaintonly) {
    ...
    function process(i, append) {
        ...
        function dohl(_hl) {
            ...
            var t2 = t.replace(
                new RegExp("(" + _hl.text_r + ")", "g"),
                "<span style='background-color:" + _hl.color + ";'>$1</span>"
            );
            var affecting = t != t2;
        ...
        div.innerHTML = t;
        if (t.match(new RegExp(selectedthread, "g")))
            div.className += " thread";

        if (append) d.appendChild(div);
        else d.insertBefore(div, d.firstChild);
    ...
    if (increment < 0) {
        ...
        for (
            var i = parseInt(d.firstChild.id) - 1;
            lefttodraw && i >= 0 && i < text.length;
            --i
        ) {
            if (process(i, false)) {
                --lefttodraw;
        ...
        for (
            var i = parseInt(d.lastChild.id) + 1;
            lefttodraw && i < text.length;
            ++i
        ) {
            if (process(i, true)) {
                --lefttodraw;
    ...
    pos.textContent =
        Math.round((parseInt(d.firstChild.id) / text.length) * 1000) / 10 + "%";
}
...
function _gfilteron() {
    ...
    if (shl && shl.filter) return true;

    for (var h in hl) {
        if (hl[h].filter) return true;
    }
...
function _getfilterelement(filter) {
    if (filter == 0) return document.getElementById("search");
...
function selectline(id) {
    ...
    if (l0) l0.style.backgroundColor = "";
    ...
    selectedlineid = id;
    if (selectedlineid > -1) _hint("line # " + (selectedlineid + 1));
...
function group() {
    ...
    if (hl.length) {
        groupwith = true;
        _hint(
            "-&gt; now select a pinned filter to group the current highlight with"
        );
    } else {
...
function restart() {
    ...
    if (!shl)
        // means: filter could not be switched back to shl or directly shl was reset
        render();
}
...
function selectfilter(filter) {
    ...
    function filterbyid(id) {
        for (var h in hl) if (hl[h].id == id) return hl[h];
    }
